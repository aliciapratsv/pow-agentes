...
function emailTemplate({ title, taskName, assignee, projectName, detail, taskGid, projectGid }) {
  ...
  return `<div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px;"><div style="background:#FF722D;padding:12px 16px;border-radius:4px;margin-bottom:20px;"><strong style="color:#020001;">${title}</strong></div><p>Tarea: <strong>${taskName}</strong></p><p>Responsable: ${assignee}</p><p>Proyecto: ${projectName}</p><p>${detail}</p><a href="${taskUrl}" style="display:inline-block;margin-top:20px;background:#020001;color:white;padding:10px 20px;border-radius:4px;text-decoration:none;">Ver tarea →</a><p style="color:#555;font-size:12px;margin-top:24px;">Taskpatrol · POW</p></div>`;
}

async function getTeamMemberGids() {
  const users = await asanaGet(`/workspaces/${WORKSPACE_GID}/users?opt_fields=gid,email`);
  return users
    .filter(u => TEAM_EMAILS.includes(u.email))
    .map(u => u.gid);
}
...
async function checkProject(project, config, teamGids) {
  ...
  const tasks = await asanaGet(
    `/projects/${project.gid}/tasks?opt_fields=gid,name,assignee.name,assignee.gid,assignee.email,due_on,modified_at,completed`
  );

  for (const task of tasks) {
    if (task.completed || !task.assignee) continue;
    if (!teamGids.includes(task.assignee.gid)) continue; // solo el equipo
